...
from .edit_contract import template_contract
...
def send_application(request, uid):
    ...
    for requirement in application.requirements.all():
        text += f"""{requirement.product.name}, кол-во: {requirement.amount};
    ...
    contract = InternalContract.objects.create(
        company=user.profile.company,
        date=dt.now(),
        contract=template_contract("internal.docx", "<INPUT>", text)
    )
    contract.save()
...
class OrderPartView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            text = f"""Поставщик: {updated_part.product.provider.name}
Товар: {updated_part.product.product.name}
Количество: {updated_part.amount}
Итоговая стоимость: {updated_part.cost}
Доставить по адресу {updated_part.address}
Получатель - {updated_part.fio}
            """

            contract = ExternalContract.objects.create(
                contract=template_contract("external.docx", "<INPUT>", text),
                date=dt.now(),
                company=user.profile.company,
                provider=updated_part.product.provider
            )
            contract.save()

            updated_part.contract = contract
            updated_part.save()
